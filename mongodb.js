...
module.exports = function(RED) {
    ...
    const forEachIteration = new Error("node-red-contrib-mongodb-var forEach iteration");
    const forEachEnd = new Error("node-red-contrib-mongodb-var forEach end");
    ...
    RED.nodes.registerType("mongodb-var", function MongoConfigNode(n) {
    ...
    RED.httpAdmin.get('/mongodb-var/vcap', function(req, res) {
        res.json(services);
    });

    RED.httpAdmin.get('/mongodb-var/operations', function(req, res) {
    ...
    function getClient(config) {
        ...
        if(allMongoConnectings){
            config.uri = allMongoConnectings.uri;
        }
        if (!poolCell) {
    ...
    RED.nodes.registerType("mongodb-var in", function MongoInputNode(n) {
        ...
        getClient(node.config).then(function(client) {
            ...
            node.on('node-red-contrib-mongodb-var handleMessage', function(msg) {
            ...
            function handlePendingMessageOnDemand() {
                while (client.queue.length > 0) {
                    ...
                    if (!targetNode.emit('node-red-contrib-mongodb-var handleMessage', pendingMessage.msg)) {
                ...
                if (client.parallelOps <= 0) {
                    return node.error("Something went wrong with node-red-contrib-mongodb-var parallel-ops count");
                }
                client.parallelOps -= 1;
            }
        }, function(err) {
        ...
        node.on('close', function() {
            ...
            node.removeAllListeners('node-red-contrib-mongodb-var handleMessage');
            if (debouncer) {
